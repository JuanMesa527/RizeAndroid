...
/**
 * Umbrales calibrados del algoritmo de press de banca.
 *
 * Las reglas biomecanicas "crudas" viven en [canonical] (banco plano frontal,
 * vista a altura de cabeza, equivalente al setup historico de pruebas en casa
 * sobre sofa). Para cada sesion en un entorno distinto, [from] aplica una
 * transformacion geometrica determinista derivada de la inclinacion del
 * torso y la perspectiva observada al alcanzar READY.
 *
 * La transformacion es analitica (proyeccion de un angulo 3D al plano de
 * imagen cuando el brazo apunta a la camara con depresion theta). No hay
 * aprendizaje ni parametros ajustados a datos: sigue siendo rule-based.
 */
data class BenchPressCalibratedThresholds(
    ...
    companion object {
        ...
        /**
         * Aplica la transformacion geometrica al perfil capturado.
         * - Grip: calibracion directa observando que ratio 2D produce el grip
         *   neutro real. Absorbe yaw de camara y FOV.
         * - Angulos de codo: proyeccion de un angulo 3D al plano de imagen
         *   cuando el brazo apunta a la camara con depresion theta:
         *     beta' = 2 * atan(tan(beta/2) * cos(theta))
         * - Sticking: la velocidad angular percibida se comprime con cos(theta).
         *   Piso a 6 deg/s para mantenerse por encima del ruido del filtro 1€.
         * - Abduccion: familia mixta cos + linear, con clamp para que el rango
         *   OK no se invierta.
         */
        fun from(profile: BenchPressEnvironmentProfile): BenchPressCalibratedThresholds {
            ...
            // Clamp: el rango OK debe mantenerse abierto al menos 25 grados.
        ...
        /**
         * Proyeccion analitica de un angulo 3D al plano de imagen 2D cuando el
         * miembro articulado apunta a la camara con depresion theta:
         *   beta_2D = 2 * atan(tan(beta/2) * cos(theta))
         * En theta = 0 devuelve beta exactamente (cero cambio). Limites numericos
         * protegidos para beta cercano a 180.
         */
